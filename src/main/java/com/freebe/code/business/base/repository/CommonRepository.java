...
public class CommonRepository {
	 ...
	 * @return
	 */
	public <T> int updateInUse(Class<T> clazz, Long id, Boolean inUse) {
		CriteriaBuilder cb = entityManager.getCriteriaBuilder();
		CriteriaUpdate<T> updateQuery = cb.createCriteriaUpdate(clazz);
		Root<T> root = updateQuery.from(clazz);
		// update
		updateQuery.set("inUse", inUse);
		// where
		Predicate predicate = cb.conjunction();
		List<Expression<Boolean>> expressions = predicate.getExpressions();
		expressions.add(cb.equal(root.get(getIdName(clazz)), id));
	 ...
	 * @return
	 */
	public <T> int updateDelete(Class<T> clazz, Long id, Boolean isDelete) {
		CriteriaBuilder cb = entityManager.getCriteriaBuilder();
		CriteriaUpdate<T> updateQuery = cb.createCriteriaUpdate(clazz);
		Root<T> root = updateQuery.from(clazz);
		// update
		updateQuery.set("isDelete", isDelete);
		// where
		Predicate predicate = cb.conjunction();
		List<Expression<Boolean>> expressions = predicate.getExpressions();
		expressions.add(cb.equal(root.get(getIdName(clazz)), id));
